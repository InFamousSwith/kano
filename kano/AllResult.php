    ...
    <html>
        ...
        <body>
			...
			<main>
				...
				if($_POST['chose'][$chooseindex]==1)
				{
					echo "<h1> Все функции по всем пользователям </h1>";
					...
					echo "<svg version='1.1' viewBox='0 0 815 815' preserveAspectRatio='xMinYMin meet' class='svg-content'>
					 ...
					 <g>
						...
						$aquery = "SELECT * FROM research ORDER BY `id` ASC";
						$aresult = mysqli_query($mysqli,$aquery);
						$anum = mysqli_num_rows($aresult);
						$bquery = "SELECT * FROM functions ORDER BY `id` ASC";
					...
					<input style='white-space: normal;' type='submit' value='Все функции по всем пользователям' id='but1'>
						</form>";
					...
					<input style='white-space: normal;' type='submit' value='Все функции по всем пользователям (без усреднения)' id='but1'>
						</form>";
					...
					<input style='white-space: normal;' type='submit' value='Все функции по одному пользователю ' id='but1'>
						</form>";
					...
					<input style='white-space: normal;' type='submit' value='Одна функция по всем пользователям' id='but1'>
						</form>";
				...
				{	
				 ...
				 if($_POST['chose'][$chooseindex]==3)
				 {
					 echo "<h1>Все функции по всем пользователям (без усреднения)</h1>";
					echo "<div class='svg-container'>";
					echo "<svg version='1.1' viewBox='0 0 815 815' preserveAspectRatio='xMinYMin meet' class='svg-content'>
					 <g>
						<rect fill='#fff' id='canvas_background' height='800' width='800' y='-1' x='-1'/>
					...
					$zap = "SELECT * FROM `user` ORDER BY `user`.`id` ASC";
					$us = mysqli_query($mysqli,$zap);
					...
					$res = mysqli_query($mysqli,"SELECT * FROM functions ORDER BY `functions`.`id` ASC");
					$skoka = mysqli_num_rows($res);
					$o = 0;
					for($t = 0;$t < $skoka;$t++)
					{
				...
				if($_POST['chose'][$chooseindex]==4)
				{
					...
					echo "'>";
					$mysqli = mysqli_connect('localhost', 'root', '', 'kano');
					$query = "SELECT * FROM functions ORDER BY `functions`.`id` ASC";
					$result = mysqli_query($mysqli,$query);
